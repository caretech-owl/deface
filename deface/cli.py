...
def cv_observer(wait, progress, frame, opath) -> bool:
    cv2.imshow(
        "Preview of anonymization results (quit by pressing Q or Escape)", frame[:, :, ::-1]
    )  # RGB -> RGB
    if cv2.waitKey(wait) & 0xFF in [ord("q"), 27]:  # 27 is the escape key code
...
def anonymize_frame(dets, frame, mask_scale, replacewith, ellipse, draw_scores, replaceimg, mosaicsize, offset):
...
def main(argv: Sequence[str] | None = None, observer: Callable | None = None):
    ...
    for ipath in ipaths:
        ...
        if enable_preview and observer is None:
            observer = partial(cv_observer, 0 if filetype == "image" else 1)
